...
function updateDeviceProfileRequest_to_chirpstack() {
    // Create a device profile.
    const updateDeviceProfile = new device_profile_pb.DeviceProfile();
    updateDeviceProfile.setName("");
    updateDeviceProfile.setDescription("");

    updateDeviceProfile.setTenantId("");

    // Create a request to update device profile.
    const createReq = new device_profile_pb.UpdateDeviceProfileRequest();
    createReq.setDeviceProfile(updateDeviceProfile);

    deviceProfileService.update(createReq, metadata, (err, resp) => {
    if (err !== null) {
        console.log(err);
        return;
    }
    console.log('Device Profile has been updated.\n' + 
    resp.toObject());
    });
}

updateDeviceProfileRequest_to_chirpstack();